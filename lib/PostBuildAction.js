...
class PostBuildAction {
	...
	_wrapWithLogger(gulpInst) {
		...
		gulpInst
			.on('err', error => this._eventBus.emit('error', error))
			...
			.on('task_err', error => {
				this._eventBus.emit('error', error.err);
				this._eventBus.emit('trace', `Task ${error.task} finished with error after ${prettyTime(error.hrDuration)}`);
			});
	}
}
