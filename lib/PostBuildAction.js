...
class PostBuildAction {
	...
	action(storeFinder, componentFinder) {
		this._logger.info('Starting assets building...');
	...
	_wrapWithLogger(gulpInst) {
		...
		gulpInst
			.on('err', error => this._logger.error(error))
			.on('task_start', args => this._logger.trace(`Starting task ${args.task}...`))
			.on('task_stop', args => this._logger.trace(`Task ${args.task} finished after ${prettyTime(args.hrDuration)}`))
			.on('task_not_found', error => this._logger.error(`Task ${error.task} not found`))
			.on('task_err', error => {
				this._logger.error(`Task ${error.task} finished with error after ${prettyTime(error.hrDuration)}: ${error.message}`);
